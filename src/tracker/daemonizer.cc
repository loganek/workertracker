...
int Daemonizer::move_to_background()
{
    ...
    if ((pid = fork()) < 0)
    {
        WT_STD_LOG(LogLevel::ERROR) << "forking child process failed!";
        return -1;
    }
    ...
    {
        WT_SYS_LOG(LogLevel::ERROR) << "Could not open PID lock file " << pid_file_name << ", exiting.";
        return -1;
    }

    if (lockf(pid_file_handle, F_TLOCK, 0) == -1)
    {
        WT_SYS_LOG(LogLevel::WARNING) << "Could not lock PID lock file " << pid_file_name << ", exiting";
        return -1;
    }
